...
async def help(ctx):
  ...
  embed.add_field(name="Music:", value="``leave``, ``join``", inline=False)
...
async def leave(ctx):
    ...
    embed=discord.Embed(title="Leave", url=botInvite, description="Left Voice Channel.", color=0xb1acdd)
    embed.set_author(name=botAuthor, url=aGithub, icon_url=aIcon)
    embed.set_thumbnail(url=bIcon)
    await ctx.send(embed=embed)
    print("Sent message for " + PREFIX + "leave")
...
async def join(ctx):
  ...
  if voice and voice.is_connected():
    ...
    embed=discord.Embed(title="Join", url=botInvite, description="Joined Voice Channel", color=0xb1acdd)
    embed.set_author(name=botAuthor, url=aGithub, icon_url=aIcon)
    embed.set_thumbnail(url=bIcon)
    await ctx.send(embed=embed)
    print("Sent message for " + PREFIX + "join")
